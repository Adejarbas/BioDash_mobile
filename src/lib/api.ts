...
const API_BASE_URL = process.env.EXPO_PUBLIC_API_URL || 'http://localhost:3003/api';
...
export async function apiRequest<T = any>(
  ...
  try {
    ...
    if (!res.ok) {
      return { success: false, error: data.message || `Erro ${res.status}: ${res.statusText}` };
    }
...
async function authRequest<T = any>(
  ...
  return apiRequest<T>(endpoint, { ...options, headers: { ...headers, ...(options.headers || {}) } });
}
...
export const markersApi = {
  ...
  save: (data: any) => authRequest(MARKERS_URL, { method: 'POST', body: JSON.stringify(data) }),
  delete: (id: string) => authRequest(`${MARKERS_URL}/${id}`, { method: 'DELETE' }),
};
...
export const indicatorsApi = {
  ...
  save: (data: {
    ...
    month: string;
    year: string;
  }) => authRequest(`${API_BASE_URL}/indicators`, { method: 'POST', body: JSON.stringify(data) }),
};
...
export const maintenanceApi = {
  fetchSchedules: () => authRequest(`${API_BASE_URL}/maintenance/schedules`, { method: 'GET' }),
  ...
  deleteSchedule: (id: string) =>
    authRequest(`${API_BASE_URL}/maintenance/schedules/${id}`, { method: 'DELETE' }),
  createSchedule: (data: { name: string; priority: string; scheduledDate: string }) =>
...
export const alertsApi = {
  ...
  create: (data: { alertLevel: string; message: string }) =>
    authRequest(`${API_BASE_URL}/alerts`, { method: 'POST', body: JSON.stringify(data) }),
};
...
export const profileApi = {
  ...
  update: (data: any) =>
    authRequest(`${API_BASE_URL}/profile`, { method: 'PUT', body: JSON.stringify(data) }),
};
